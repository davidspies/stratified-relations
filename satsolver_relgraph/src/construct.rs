...
impl RelGraph {
    pub fn construct(context: &mut CreationContext) -> Self {
        ...
        let reduced_rules = rules
            ...
            .swaps()
            .antijoin(assigned.get().map_(Not::not))
            .swaps()
            .collect();
        let reduced_rule_inds = reduced_rules.get().fsts_().save();
        ...
        let unit_rules = rule_sizes.filter(|&(_, c)| c == 1).fsts_();
        let unit_literal_causes = reduced_rules
            .get()
            .semijoin(unit_rules)
            .cartesian_product(level)
            .map_(|((ri, l), lvl)| (l, LiteralCause::Propogated(ri, lvl)))
            .collect();
        let unit_literals = unit_literal_causes.get().fsts_().save();
        let conflict_literals = unit_literals
            .get()
            .intersection(unit_literals.get().map_(Not::not))
            .collect();
        ...
        context.set_feedback(unit_literal_causes.get().fsts_(), assign_input.clone());
        let use_counts = reduced_rules.get().snds_().counts();
        let next_literal = context.output(use_counts.swaps().global_max().snds_().dynamic());
        Self {
            rules_input,
            level_input,
            assign_input,
